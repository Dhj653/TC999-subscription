...
async def move_files(req: MoveRequest):
    """执行文件移动，受测试模式开关控制。
    内部按 FOLDER_CREATE_MIN_VIDEOS 阈值仅移动满足条件的分组。
    """
    ...
    skipped = sum(1 for r in results if not r.get("moved") and not r.get("test_mode"))
    msg = (f"已移动 {moved_count} 个文件（跳过 {skipped} 个单人视频）"
           if not req.test_mode
           else f"[预览] 拟移动 {preview_count} 个文件，跳过 {skipped} 个单人视频")
...
async def reprocess_single(req: ReprocessSingleRequest):
    ...
    return ApiResponse(success=True, message=res["message"],
                       data={"mapping_id": req.mapping_id})
